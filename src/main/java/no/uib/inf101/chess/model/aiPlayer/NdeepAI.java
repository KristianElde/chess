...
public class NdeepAI implements AIPlayer{
    ...
    private int testBranch(Move candidateMove, int depth, ChessBoard board) {
        ...
        Piece responseCapturedPiece = board.movePiece(responseMove.from(), responseMove.to(), responseMove.getPieceToMove());
        if (capturedPiece != null) {
        ...
        board.resetStateVariablesAfterMove(responseMove.from(), responseMove.to(), responseMove.from().getPiece(), responseCapturedPiece, isCheckBeforeResponse, allowCastlingBeforeResponse);
        board.undoMove(candidateMove.from(), candidateMove.to(), candidateMove.from().getPiece(), capturedPiece);
        board.resetStateVariablesAfterMove(candidateMove.from(), candidateMove.to(), candidateMove.from().getPiece(), capturedPiece, isCheck, allowCastling);
